  ...
  template: `
    ...
    <div>
      <ul class="nav nav-tabs">
        <li class="nav-item">
          <a class="nav-link active"><i class="{{ abcForms.btnNew.icon }}"></i> Editar Categoria</a>
        </li>
      </ul>
        ...
        <div class="form-group col-md-2 required">
          ...
          <div class="input-group input-group-sm input-group-rounded">
            <input type="text" class="form-control form-control-sm" formControlName="descripccion"
                   id="descripccion"
                   placeholder="descripccion de la Categoria">
          </div>
          <app-form-validate-errors [group]="categoriForm"
        ...
        <div class="form-group col-md-2 required">
          ...
          <div class="input-group input-group-sm input-group-rounded">
            <input type="text" class="form-control form-control-sm" formControlName="etiqueta"
                   id="etiqueta"
                   placeholder="etiqueta de la Categoria">
          </div>
          <app-form-validate-errors [group]="categoriForm"
...
export class CategoriEditComponent implements OnInit {
  abcForms: any;
  public idCategori: number = 0;
  public categori = new Categori();
  ...
  ngOnInit() {
    ...
    this.route.params.subscribe(res => {
      this.idCategori = parseInt(res['idCategori']);
      this.categoriForm.value.id = this.idCategori;
      this.getCategoriById(this.idCategori);
    });
    console.log("app-categori-new");
  }

  getCategoriById(idCategori: number): void {
    this.categorisService.getById$(idCategori).subscribe(response => {
      this.categori = response;
      // @ts-ignore
      this.categoriForm.patchValue(this.categori);
    });
  }
