...
@Component({
  selector: 'app-pedid-list',
  template: `
      <div class="float-end">
          <button type="button" (click)="goNew()" class="btn-gm-danger">
              <span class="{{ abcForms.btnNew.icon }} lamb-icon"></span> {{ abcForms.btnNew.label }} Pedido
          </button>
      </div>
      <div class="responsive-table">
          <table class="table table-lg table-hover table-striped table-sm">
              ...
              <tr *ngFor="let v of pedids ; let i=index">
                  <th scope="row">{{i + 1}}</th>
                  
                  <td data-title="descripcion">{{v.descripcion}}</td>
                  <td data-title="numero">{{v.numero}}</td>
...
export class PedidListComponent implements OnInit {
  abcForms: any;
  @Input() pedids: Pedid[] = [];
